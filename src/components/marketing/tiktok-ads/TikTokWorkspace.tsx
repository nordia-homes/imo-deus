...
import { Button, EmptyState, MetricCard, StatusBadge, StudioEyebrow, PermissionChip } from './StudioPrimitives';
...
export default function TikTokWorkspace({ initialTab = 'overview' }: { initialTab?: Tab }) {
  ...
  return <main className="tt-design tt-workspace space-y-6">
    <header className="tt-hero"><div><StudioEyebrow>MARKETING · AGENȚIE</StudioEyebrow><h1>TikTok <em>Studio</em></h1><p>Proprietăți care atrag priviri.<br />Videoclipuri și campanii care deschid conversații.</p><div className="tt-hero-actions"><Button disabled={!workspace || busy} onClick={() => newDraft()}><Plus size={17} />Creează reclamă</Button><Button variant="outline" onClick={() => setTab('videos')}><Clapperboard size={17} />Videoclipuri</Button></div></div><div className="tt-hero-visual" aria-hidden="true"><div className="tt-orbit" /><div className="tt-orbit tt-orbit--two" /><div className="tt-brand-emblem"><TikTokIcon /></div><span className="tt-visual-tag">REAL ESTATE. REAL IMPACT.</span></div></header>
    ...
    <nav aria-label="Secțiuni TikTok" className="tt-nav">{tabs.map(([key, label]) => <Button key={key} variant={tab === key ? 'default' : 'ghost'} aria-current={tab === key ? 'page' : undefined} onClick={() => setTab(key)}>{key === "overview" ? <LayoutDashboard /> : key === "ads" ? <Megaphone /> : key === "videos" ? <Clapperboard /> : <Settings2 />}{label}</Button>)}</nav>
    {!workspace ? <p role="status">{notice ? 'Pagina nu a putut fi încărcată.' : 'Se încarcă spațiul TikTok…'}</p> : <>
      ...
      </section><section className={panelClass}><h2 className="font-semibold">Drafturi salvate</h2>{drafts.length ? <div className="mt-3 grid gap-3 md:grid-cols-3">{drafts.map(item => <button key={item.id} className="tt-draft-card" onClick={() => setDraft(item)}><p className="font-medium">{item.data.name || 'Reclamă fără nume'}</p><p className="text-xs text-slate-500">Versiunea {item.version} · Continuă editarea</p></button>)}</div> : <p className="mt-3 text-sm text-slate-500">Drafturile sunt salvate în agenție, fără a cheltui buget.</p>}</section></div>}
