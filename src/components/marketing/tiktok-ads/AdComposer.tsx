...
export function AdComposer({ api, workspace, advertiserId, initial, onClose, onCreated }: { api: Api; workspace: Workspace; advertiserId: string; initial: SavedAdDraft; onClose: () => void; onCreated: () => void }) {
  ...
  async function create() {
    ...
    try {
      ...
      if (saveTimer.current) clearTimeout(saveTimer.current);
      await save(draft);
      if (draft.mode === 'video' && !draft.adgroupId && !advertiser?.timezone) throw new Error('Sincronizează fusul orar al contului înainte de creare.');
      ...
      const missing = (draft.mode === 'video' ? [['CAMPAIGN_CREATE', inputs.campaign], ['ADGROUP_CREATE', inputs.adGroup], ['CREATIVE_UPLOAD', inputs.video], ['AD_CREATE', inputs.ad]] as const : [['AD_CREATE', inputs.ad]] as const).filter(([cap]) => !draft.adgroupId || !['CAMPAIGN_CREATE', 'ADGROUP_CREATE'].includes(cap)).flatMap(([cap, value]) => schemaMissing(workspace.schemas[cap], value, supplied));
      ...
      const payload = draft.mode === 'video' ? { adsOnly: true, tiktokAccountId: draft.identityId, permissionToolInput: {}, ...(!draft.adgroupId ? { campaign: inputs.campaign, adGroup: inputs.adGroup } : {}), video: { mediaAssetId: draft.assetId, toolInput: inputs.video }, ad: inputs.ad } : { tiktokAccountId: draft.identityId, permissionToolInput: {}, ad: inputs.ad };
    ...
    <DialogPrimitive.Content className="tt-design tt-composer" onInteractOutside={event => event.preventDefault()} onEscapeKeyDown={event => { if (busy) event.preventDefault(); }}>
      ...
      <div className="tt-composer-progress" aria-hidden="true"><span style={{ width: `${(step + 1) * 25}%` }} /></div><div className="tt-composer-layout">
        ...
        <section className="tt-composer-form"><div className="tt-form-heading"><span>PASUL 0{step + 1} / 04</span><h3>{stepTitles[step]}</h3><p>{stepDescriptions[step]}</p></div><div className="tt-fields">
          {step === 0 && <>
            <Field label="Structura reclamei"><select aria-label="Structura reclamei" className={inputClass} value={existingGroup ? 'existing' : 'new'} disabled={busy || !!result || draft.mode === 'post'} onChange={event => { setExistingGroup(event.target.value === 'existing'); change({ adgroupId: '' }); }}><option value="new">Flux rapid · campanie + grup + reclamă</option><option value="existing">Adaugă reclama într-un grup existent</option></select></Field>
            {existingGroup && <><Button variant="outline" disabled={busy || !!result} onClick={() => void loadOptions('adgroup')}>Încarcă grupurile disponibile</Button><select aria-label="Grup țintă" className={inputClass} value={draft.adgroupId} disabled={busy || !!result} onChange={event => change({ adgroupId: event.target.value })}><option value="">Selectează grupul</option>{draft.adgroupId && !groups.some(row => row.id === draft.adgroupId) && <option value={draft.adgroupId}>Grup selectat · {draft.adgroupId}</option>}{groups.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><p className="text-xs text-slate-500">Se păstrează campania, audiența, programul și bugetul grupului.</p></>}
