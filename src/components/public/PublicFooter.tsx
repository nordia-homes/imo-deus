...
export function PublicFooter() {
    ...
    const isClassicTheme = themePreset === 'classic';
        ...
        <footer
            className={cn(
                ...
                isClassicTheme
                    ...
                    : isAgentfinderTheme
                        ? 'border-t border-slate-200 bg-[linear-gradient(180deg,#f8fbff_0%,#eef6ff_100%)]'
                ...
                <div
                    className={cn(
                        ...
                        isClassicTheme
                            ...
                            : isAgentfinderTheme
                                ? 'border border-slate-200/80 bg-white/82'
                                : 'border border-white/8 bg-white/[0.03]'
                    )}
                >
                    <Copyright className={cn('h-4 w-4', isClassicTheme ? 'text-emerald-300' : isAgentfinderTheme ? 'text-sky-700' : 'text-[var(--public-accent-soft)]')} />
                    <p className={cn('text-sm font-medium', isClassicTheme ? 'text-stone-100' : isAgentfinderTheme ? 'text-slate-700' : 'text-white/90')}>
                        2026 drepturi rezervate Nordia Homes
                    </p>
                    {agencyId ? (
                        <div className={cn('flex items-center gap-2 text-sm', isClassicTheme ? 'text-stone-300' : isAgentfinderTheme ? 'text-slate-500' : 'text-white/65')}>
                            <Link
                                href={publicPath('/termeni-si-conditii')}
                                className={cn('transition-colors', isClassicTheme ? 'hover:text-emerald-300' : isAgentfinderTheme ? 'hover:text-sky-700' : 'hover:text-[var(--public-accent-soft)]')}
                            >
                                Termeni si conditii
                            </Link>
                            <span className={cn(isClassicTheme ? 'text-emerald-400/70' : isAgentfinderTheme ? 'text-slate-400' : 'text-[var(--public-accent)] opacity-60')}>•</span>
                            <Link
