        ...
        <header className="px-4 md:px-6 lg:px-0 py-4 border-b bg-background/95 backdrop-blur-sm lg:bg-transparent lg:border-white/10 lg:mb-2">
            <div className="flex h-full flex-col gap-4 lg:grid lg:grid-cols-12 lg:items-start lg:gap-8">
                ...
                <div className="flex flex-wrap items-center gap-2 lg:col-span-4 lg:grid lg:grid-rows-[4rem_3.5rem] lg:gap-3">
                    <div className={`${ACTION_CARD_CLASSNAME} w-full rounded-[1.7rem] p-4 lg:flex lg:h-16 lg:items-center lg:p-2`}>
                        <div className="flex w-full items-center gap-3">
                            <Select onValueChange={(value) => handleStatusChange(value as Property['status'])} defaultValue={property.status}>
                                <SelectTrigger className="h-12 min-w-[132px] rounded-[1.15rem] border border-white/8 bg-[#1a3046] px-4 text-sm font-semibold text-emerald-200 hover:bg-[#203850] lg:h-10">
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                            ...
                            <Button className={`h-12 rounded-[1.15rem] px-5 text-white lg:h-10 ${ACTION_PILL_CLASSNAME}`} onClick={onTriggerAddViewing}>
                                <CalendarCheck className="mr-2 h-4 w-4"/> 
                                Vizionare
                            </Button>
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button variant="outline" size="icon" className={`h-12 w-12 rounded-[1.15rem] text-white lg:h-10 lg:w-10 ${ACTION_PILL_CLASSNAME}`}>
                    ...
                    <div className={`${ACTION_CARD_CLASSNAME} hidden h-14 w-full min-w-0 items-center gap-3 rounded-[1.45rem] px-4 lg:flex`}>
                        <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${ACTION_PILL_CLASSNAME}`}>
                            <MapPin className="h-4 w-4 text-rose-300" />
                        </span>
                        <div className="min-w-0 text-left">
                            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/45">Adresă proprietate</p>
                            <p className="truncate text-sm font-semibold text-white" title={displayAddress}>{displayAddress}</p>
                        </div>
                    </div>
                </div>
            </div>
