...
export function PreferencesFormCard({ contact, agency }: PreferencesFormCardProps) {
  ...
  return (
    <Card className="agentfinder-client-portal-card mx-2 rounded-2xl shadow-2xl bg-[#152A47] border-none text-white lg:mx-0">
      <CardHeader className="p-4 pb-2 lg:pb-2">
        <CardTitle className="agentfinder-client-portal-title flex items-center gap-2 text-base text-white">
          <Send className="agentfinder-client-portal-title-icon h-5 w-5 text-primary" />
      ...
      <CardContent className="space-y-3 p-4 pt-0 lg:p-6 lg:pt-0">
        ...
        {preferencesLink ? (
          <>
            <div>
              <Label htmlFor="preferences-link" className="agentfinder-client-portal-label text-xs text-white/70">Link Formular</Label>
              <div className="flex gap-2 mt-1">
                <Input id="preferences-link" readOnly value={preferencesLink} className="agentfinder-client-portal-input bg-white/10 border-white/20 h-9" />
                ...
                <Button variant="secondary" size="icon" onClick={handleGenerateLink} disabled={isLoading} className="agentfinder-client-portal-icon-button agentfinder-sidebar-button agentfinder-sidebar-button--icon h-9 w-9 shrink-0">
                  {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
                </Button>
              </div>
            </div>
             <Button size="sm" variant="secondary" onClick={() => window.open(preferencesLink, '_blank')} disabled={isLoading} className="agentfinder-client-portal-button agentfinder-client-portal-button--open agentfinder-sidebar-button agentfinder-sidebar-button--full w-full">
                <LinkIcon className="mr-2 h-4 w-4" /> Deschide Formular
              </Button>
          </>
        ) : (
          <Button onClick={handleGenerateLink} disabled={isLoading} className="agentfinder-client-portal-button agentfinder-sidebar-button agentfinder-sidebar-button--full h-11 w-full rounded-full">
