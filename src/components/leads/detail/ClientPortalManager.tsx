          ...
          <>
            <div>
              ...
              <div className="flex gap-2 mt-1">
                ...
                <Button variant="secondary" size="icon" onClick={handleCopy} className="agentfinder-button-tertiary h-9 w-9 shrink-0 bg-white/20 hover:bg-white/30">
                  {copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                </Button>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Button size="sm" variant="secondary" onClick={() => window.open(portalLink, '_blank')} disabled={isLoading} className="agentfinder-button-secondary bg-white/90 text-black hover:bg-white flex-1">
                <LinkIcon className="mr-2 h-4 w-4" /> Deschide
              </Button>
              <Button size="icon" variant="secondary" onClick={() => handlePortalAction('regenerate')} disabled={isLoading} className="agentfinder-button-tertiary bg-white/20 hover:bg-white/30">
                {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
              </Button>
              <Button size="icon" variant="destructive" onClick={() => handlePortalAction('deactivate')} disabled={isLoading} className="agentfinder-button-tertiary">
                 {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
              </Button>
            </div>
          </>
        ) : (
          <Button onClick={() => handlePortalAction('activate')} disabled={isLoading} className="agentfinder-button-primary h-11 w-full rounded-full bg-[#1f4b7a] text-white hover:bg-[#24588f]">
