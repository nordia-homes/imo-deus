...
export function EditLeadDialog({ properties, contact, isOpen, onOpenChange, onUpdateContact }: EditLeadDialogProps) {
  ...
  const form = useForm<z.infer<typeof cumparatorSchema>>({
    ...
    defaultValues: {
      ...
      status: contact.status || 'Nou',
      description: contact.description || '',
      city: contact.city || '',
      priority: contact.priority || 'Medie',
      agentId: contact.agentId || 'unassigned',
      sourcePropertyId: contact.sourcePropertyId || 'none',
    },
  });
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setSelectedZones(contact.zones || []);
    }
  }, [contact, isOpen, form]);
