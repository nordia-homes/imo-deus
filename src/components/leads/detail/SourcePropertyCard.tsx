...
export function SourcePropertyCard({ property, isLoading, allProperties, onUpdateContact }: SourcePropertyCardProps) {
    ...
    if (isLoading) {
        return (
            <Card className="mx-2 lg:mx-0 rounded-2xl shadow-2xl bg-[#152A47] lg:bg-card">
             ...
             <Card className="mx-2 lg:mx-0 rounded-2xl shadow-2xl bg-[#152A47] lg:bg-card text-white lg:text-inherit">
                ...
                <CardContent>
                    <Select onValueChange={handleSelectChange} defaultValue={property?.id || 'none'}>
                        <SelectTrigger className="bg-white/10 lg:bg-background border-white/20 lg:border-input">
                    ...
                    <Button variant="ghost" onClick={() => setIsEditing(false)} className="mt-2 w-full text-white/80 lg:text-inherit">Anulează</Button>
                </CardContent>
            </Card>
        )
    }
    ...
    return (
        <Card className="mx-2 lg:mx-0 rounded-2xl shadow-2xl bg-[#152A47] lg:bg-card text-white lg:text-inherit">
            ...
            <CardContent className="p-4 pt-0">
                {!property ? (
                    <div className="text-center text-white/70 lg:text-muted-foreground py-4">
                         <p className="text-sm mb-2">Nicio proprietate sursă asociată.</p>
                         <Button size="sm" variant="secondary" onClick={() => setIsEditing(true)} className="bg-white/10 lg:bg-secondary hover:bg-white/20 lg:hover:bg-secondary/80 text-white lg:text-secondary-foreground">Asociază o proprietate</Button>
                    </div>
                ) : (
                    <Link href={`/properties/${property.id}`} className="block group">
                        <div className="flex gap-4 items-center">
                            ...
                            <Link2 className="h-5 w-5 text-white/70 lg:text-muted-foreground group-hover:text-white lg:group-hover:text-primary" />
                        </div>
                    </Link>
                )}
            </CardContent>
