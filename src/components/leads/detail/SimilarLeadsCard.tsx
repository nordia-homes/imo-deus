        ...
        <Card className="rounded-2xl shadow-2xl">
            ...
            <CardContent className="space-y-3">
                {cumparatori.map(cumparator => (
                    <Link href={`/leads/${cumparator.id}`} key={cumparator.id} className="flex items-center justify-between p-3 rounded-lg border hover:bg-accent group">
                        <div>
                            <p className="font-semibold text-sm group-hover:text-primary">{cumparator.name}</p>
                            <p className="text-xs text-muted-foreground">Buget: €{cumparator.budget?.toLocaleString()}</p>
                        </div>
