...
export async function generateTikTokStudioCreativeBrief(input: CreativeBriefInput): Promise<TikTokStudioCreativeBrief> {
  ...
  const prompt = [
    ...
    'Daca primesti hook, incaperi si descrieri scrise de utilizator, transforma-le intr-o compunere cursiva si pastreaza ordinea incaperilor.',
    'Hook-ul trebuie sa contina fraza de impact si sumar descriptiv, fara pret. Pretul poate aparea doar la finalul scriptului.',
    'Storyboard-ul trebuie sa sincronizeze fiecare scena cu fragmentul de voiceover care descrie exact incaperea sau momentul vizual din scena.',
