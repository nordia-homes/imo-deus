...
export function renderPropertyPresentationHtml(input: PropertyPresentationTemplateInput) {
  ...
  const title = shorten(property.title, 86) || `${property.propertyType || 'Proprietate'} ${property.rooms || ''} camere`;
  const titleSize = title.length > 66 ? 25 : title.length > 46 ? 29 : 34;
  ...
  return `<!doctype html><html lang="ro"><head><meta charset="utf-8" /><title>${escapeHtml(property.title)} — prezentare</title><style>
    ...
    .gallery{display:grid;grid-template-columns:repeat(4,1fr);gap:2mm;height:43mm;margin-top:2.5mm}.gallery-card{display:grid;grid-template-rows:1fr 6mm 7mm;min-width:0;text-align:center}.gallery-card .photo{border-radius:2.5mm}.gallery-card strong{position:relative;z-index:1;align-self:center;margin:-2mm 2mm 0;border-radius:99px;background:#a47c4e;color:#fff;padding:1.4mm 1mm;font-size:6px;letter-spacing:.04em;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.gallery-card span{align-self:end;color:#51483f;font-family:Georgia,serif;font-size:6.2px;line-height:1.15}
