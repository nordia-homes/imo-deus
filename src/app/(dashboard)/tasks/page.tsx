...
export default function TasksPage() {
    ...
    return (
        <div className="flex h-full flex-col gap-5 overflow-hidden bg-[#0B1730] p-2 text-white lg:gap-6 lg:p-4">
            <section className="relative overflow-hidden rounded-[30px] border border-white/10 bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.28),_transparent_35%),linear-gradient(135deg,_rgba(14,31,56,0.98),_rgba(11,23,48,0.94)_55%,_rgba(17,57,100,0.88))] px-5 py-5 shadow-[0_24px_80px_rgba(0,0,0,0.28)] lg:px-7 lg:py-7">
                ...
                <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                    <div className="max-w-2xl">
                        <span className="inline-flex rounded-full border border-sky-300/20 bg-sky-400/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em] text-sky-100/80">
                            Workspace task-uri
                        </span>
                    ...
                    <AddTaskDialog onAddTask={handleAddTask} contacts={contacts || []}>
                        <Button className="h-12 rounded-2xl border border-sky-300/20 bg-sky-400/[0.15] px-5 text-sm font-semibold text-white shadow-[0_12px_30px_rgba(59,130,246,0.18)] hover:bg-sky-400/[0.25]">
                ...
                <div className="relative mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
                    {[
                        ...
                        <div
                            key={item.label}
                            className="rounded-[24px] border border-white/10 bg-white/[0.06] px-4 py-4 backdrop-blur-sm"
                        >
            ...
            <Tabs defaultValue="board" className="flex flex-1 flex-col overflow-hidden">
                <div className="rounded-[28px] border border-white/10 bg-[#12213E]/90 p-3 shadow-[0_20px_60px_rgba(0,0,0,0.22)]">
                    <TabsList className={cn(
                        "grid h-auto w-full grid-cols-3 rounded-[20px] bg-white/5 p-1 text-white/60"
                    )}>
                        <TabsTrigger value="board" className="flex items-center gap-2 rounded-2xl py-3 text-sm data-[state=active]:bg-white data-[state=active]:text-slate-950">
                            <LayoutGrid className="h-4 w-4" />
                            Panou
                        </TabsTrigger>
                        <TabsTrigger value="list" className="flex items-center gap-2 rounded-2xl py-3 text-sm data-[state=active]:bg-white data-[state=active]:text-slate-950">
                            <ListTodo className="h-4 w-4" />
                            Listă
                        </TabsTrigger>
                        <TabsTrigger value="calendar" className="flex items-center gap-2 rounded-2xl py-3 text-sm data-[state=active]:bg-white data-[state=active]:text-slate-950">
