...
function AgentCard({ agent, whatsappHref, isAgentfinderTheme = false }: { agent: AgentInfo; whatsappHref?: string | null; isAgentfinderTheme?: boolean }) {
    ...
    return (
        <div className={cn("flex items-center justify-between rounded-[1.5rem] p-4", isAgentfinderTheme ? "border border-slate-200 bg-white/88" : "border border-white/10 bg-[#18191d]")}>
            <div className="flex items-center gap-3">
                 <Avatar className="h-12 w-12">
                    <AvatarImage src={agent.avatarUrl || undefined} alt={agent.name || 'Agent'}/>
                    <AvatarFallback className={cn(isAgentfinderTheme ? "bg-sky-50 text-sky-700" : "bg-[#22c55e]/15 text-[#86efac]")}>{getInitials(agent.name)}</AvatarFallback>
                </Avatar>
                <div>
                    <p className={cn("text-xs", isAgentfinderTheme ? "text-slate-500" : "text-stone-500")}>Consultantul tau</p>
                    <p className={cn("text-base font-semibold", isAgentfinderTheme ? "text-slate-950" : "text-stone-50")}>{agent.name}</p>
                    {agent.phone && <p className={cn("text-sm", isAgentfinderTheme ? "text-slate-500" : "text-stone-400")}>{agent.phone}</p>}
                </div>
            </div>
            <div className="flex items-center">
                {agent.phone && (
                    <Button variant="ghost" size="icon" className={cn("h-9 w-9 rounded-full", isAgentfinderTheme ? "text-slate-500 hover:bg-slate-100 hover:text-slate-950" : "text-stone-300 hover:bg-white/5 hover:text-stone-50")} asChild>
                ...
                {sanitizedPhone && whatsappHref && (
                    <Button variant="ghost" size="icon" className={cn("h-9 w-9 rounded-full", isAgentfinderTheme ? "text-sky-700 hover:bg-sky-50 hover:text-sky-800" : "text-stone-300 hover:bg-white/5 hover:text-stone-50")} asChild>
...
function PublicScheduleViewingCard({ property, agent, agencyId, whatsappHref, isAgentfinderTheme = false }: { property: Property, agent: AgentInfo, agencyId: string, whatsappHref?: string | null, isAgentfinderTheme?: boolean }) {
    ...
    return (
        <Card className={cn("rounded-[2rem] border shadow-[0_28px_80px_-40px_rgba(0,0,0,0.85)]", isAgentfinderTheme ? "public-premium-panel text-slate-950" : "border-white/10 bg-[#101113]/95 text-stone-100")}>
            <CardContent className="space-y-4 pt-5">
                <div className={cn("overflow-hidden rounded-[1.5rem] border", isAgentfinderTheme ? "border-slate-200 bg-white" : "border-white/10 bg-[#0b0f0d]")}>
                    <PriceStatusCard property={property} isMobile={false} isAgentfinderTheme={isAgentfinderTheme} />
                </div>
                <AgentCard agent={agent} whatsappHref={whatsappHref} isAgentfinderTheme={isAgentfinderTheme} />
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <FormField control={form.control} name="name" render={({ field }) => ( <FormItem><FormLabel className={cn(isAgentfinderTheme ? "text-slate-700" : "text-stone-300")}>Nume</FormLabel><FormControl><Input {...field} placeholder="Numele tau" className={cn(isAgentfinderTheme ? "border-slate-200 bg-white text-slate-950 placeholder:text-slate-400" : "border-white/10 bg-[#18191d] text-stone-100 placeholder:text-stone-500")} /></FormControl><FormMessage /></FormItem> )} />
                        <FormField control={form.control} name="phone" render={({ field }) => ( <FormItem><FormLabel className={cn(isAgentfinderTheme ? "text-slate-700" : "text-stone-300")}>Telefon</FormLabel><FormControl><Input {...field} placeholder="0712 345 678" className={cn(isAgentfinderTheme ? "border-slate-200 bg-white text-slate-950 placeholder:text-slate-400" : "border-white/10 bg-[#18191d] text-stone-100 placeholder:text-stone-500")} /></FormControl><FormMessage /></FormItem> )} />
                        <FormField control={form.control} name="email" render={({ field }) => ( <FormItem><FormLabel className={cn(isAgentfinderTheme ? "text-slate-700" : "text-stone-300")}>Email</FormLabel><FormControl><Input {...field} type="email" placeholder="email@exemplu.com" className={cn(isAgentfinderTheme ? "border-slate-200 bg-white text-slate-950 placeholder:text-slate-400" : "border-white/10 bg-[#18191d] text-stone-100 placeholder:text-stone-500")} /></FormControl><FormMessage /></FormItem> )} />
                        <FormField control={form.control} name="message" render={({ field }) => ( <FormItem><FormLabel className={cn(isAgentfinderTheme ? "text-slate-700" : "text-stone-300")}>Mesaj (Optional)</FormLabel><FormControl><Textarea {...field} placeholder="As dori mai multe detalii despre..." className={cn(isAgentfinderTheme ? "border-slate-200 bg-white text-slate-950 placeholder:text-slate-400" : "border-white/10 bg-[#18191d] text-stone-100 placeholder:text-stone-500")} /></FormControl><FormMessage /></FormItem> )} />
                        <Button type="submit" disabled={isSubmitting} className={cn("w-full rounded-full", isAgentfinderTheme ? "public-premium-primary-button" : "bg-[#22c55e] text-black hover:bg-[#4ade80]")}>
    ...
    if (isMobile) {
        return (
          <div className={cn("min-h-screen bg-transparent pb-24", isAgentfinderTheme ? "text-slate-950" : "text-stone-100")}>
             <div className={cn("space-y-0", isAgentfinderTheme && "pt-2")}>
                ...
                <div className="space-y-4 px-2">
                    <div className={cn(isAgentfinderTheme ? "space-y-0" : "space-y-4")}>
                        <Card className={cn("mx-[-0.5rem] -mb-4 overflow-hidden rounded-b-none rounded-t-none border-0 shadow-none", isAgentfinderTheme ? "public-premium-panel" : "bg-[#0b0f0d]")}>
                            <CardContent className="space-y-3 p-4">
                                <div className={cn("flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-sm font-medium", isAgentfinderTheme ? "text-slate-600" : "text-stone-200")}>
                                    {property.rooms ? <span className="whitespace-nowrap">{property.rooms} camere</span> : null}
                                    {property.rooms && displaySurface ? <span className={cn("h-1.5 w-1.5 rounded-full", isAgentfinderTheme ? "bg-sky-500" : "bg-[#22c55e]")} /> : null}
                                    {displaySurface ? <span className="whitespace-nowrap">{displaySurface} mp</span> : null}
                                    {(property.rooms || displaySurface) && property.constructionYear ? <span className={cn("h-1.5 w-1.5 rounded-full", isAgentfinderTheme ? "bg-sky-500" : "bg-[#22c55e]")} /> : null}
                                    {property.constructionYear ? <span className="whitespace-nowrap">An {property.constructionYear}</span> : null}
                                    {(property.rooms || displaySurface || property.constructionYear) && property.floor ? <span className={cn("h-1.5 w-1.5 rounded-full", isAgentfinderTheme ? "bg-sky-500" : "bg-[#22c55e]")} /> : null}
                                    {property.floor ? <span className="whitespace-nowrap">Etaj {property.floor}</span> : null}
                                </div>

                                <div className={cn("space-y-1 pt-3 text-center", isAgentfinderTheme ? "border-t border-slate-200/80" : "border-t border-white/10")}>
                                    <CardTitle className={cn("text-xl font-bold", isAgentfinderTheme ? "text-slate-950" : "text-stone-50")}>{property.title}</CardTitle>
                                    <CardDescription className={cn("text-sm", isAgentfinderTheme ? "text-slate-500" : "text-stone-400")}>{property.address}</CardDescription>
                                </div>
                            </CardContent>
                        </Card>
                        
                        <div className={cn(isAgentfinderTheme ? "mx-[-0.5rem] overflow-hidden rounded-b-[2rem] rounded-t-none border-x border-b border-slate-200 bg-white/90 shadow-[0_22px_44px_-28px_rgba(15,23,42,0.28)]" : "")}>
                            <PriceStatusCard property={property} isMobile={isMobile} isAgentfinderTheme={isAgentfinderTheme} />
                        </div>
                    </div>
                    ...
                    <Card className={cn(financeCardClassName, isAgentfinderTheme && "public-premium-panel border-slate-200 bg-none")}>
                        <CardContent className="p-5">
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h3 className={cn("text-2xl font-semibold tracking-tight", isAgentfinderTheme ? "text-slate-950" : "text-white")}>
                                        Continua cautarea in portofoliul complet al agentiei.
                                    </h3>
                                    <p className={cn("text-sm leading-7 md:text-base", isAgentfinderTheme ? "text-slate-600" : "text-emerald-50/85")}>
                                ...
                                <Button asChild className={cn("w-full rounded-full", isAgentfinderTheme ? "public-premium-primary-button" : "bg-[#22c55e] text-black hover:bg-[#4ade80]")}>
             ...
             <div className={cn("fixed bottom-0 left-0 right-0 z-50 p-3 shadow-[0_-12px_40px_-18px_rgba(0,0,0,0.7)] backdrop-blur-xl", isAgentfinderTheme ? "border-t border-slate-200 bg-white/92" : "border-t border-white/10 bg-black/88")}>
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <Avatar className="h-11 w-11">
                            ...
                            <AvatarFallback className={cn(isAgentfinderTheme ? "bg-sky-50 text-sky-700" : "bg-[#22c55e]/15 text-[#86efac]")}>{getInitials(agentForCard.name)}</AvatarFallback>
                        </Avatar>
                        <div>
                            <p className={cn("text-xs", isAgentfinderTheme ? "text-slate-500" : "text-stone-500")}>Contacteaza agentul</p>
                            <p className={cn("text-base font-semibold", isAgentfinderTheme ? "text-slate-950" : "text-stone-50")}>{agentForCard.name}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        {agentForCard.phone && (
                            <Button variant="outline" size="icon" className={cn("h-12 w-12 rounded-full", isAgentfinderTheme ? "border-slate-200 bg-white text-slate-700 hover:bg-slate-50" : "border-white/10 bg-white/5 text-stone-100 hover:bg-white/10")} asChild>
             ...
             <div className="container mx-auto px-4 py-8">
                <main className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                        ...
                        <Card className={cn(financeCardClassName, isAgentfinderTheme && "public-premium-panel border-slate-200 bg-none")}>
                            <CardContent className="p-6">
                                <div className="space-y-4">
                                    ...
                                    <Button asChild className={cn("rounded-full", isAgentfinderTheme ? "public-premium-primary-button" : "bg-[#22c55e] text-black hover:bg-[#4ade80]")}>
                                        <Link href={publicPath('/properties')} className="inline-flex items-center justify-center gap-2">
                                            Vezi toate proprietatile
                                            <ArrowRight className="h-4 w-4" />
                                        </Link>
                    ...
                    <div className="col-span-12 lg:col-span-4 lg:sticky top-24 space-y-4">
                         <PublicScheduleViewingCard property={property} agent={agentForCard} agencyId={agencyId} whatsappHref={whatsappHref} isAgentfinderTheme={isAgentfinderTheme} />
                    </div>
                </main>
             </div>
        </div>
