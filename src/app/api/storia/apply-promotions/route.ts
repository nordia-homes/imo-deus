...
function formatError(error: unknown) {
  if (error && typeof error === 'object' && 'status' in error) {
    ...
    const message = error instanceof Error ? error.message : 'A aparut o eroare neasteptata la aplicarea promovarilor Storia.';
    return { status, message };
  }
  if (error instanceof Error) {
    return { status: 500, message: error.message };
  }
  return { status: 500, message: 'A aparut o eroare neasteptata la aplicarea promovarilor Storia.' };
}

export async function POST(request: NextRequest) {
  try {
    const [{ requireAgencyUserFromBearerToken }, { applyStoriaPromotions }] = await Promise.all([
