...
function formatError(error: unknown) {
  if (error && typeof error === 'object' && 'status' in error) {
    ...
    const message = error instanceof Error ? error.message : 'A aparut o eroare neasteptata la publicarea in Storia.';
    return { status, message, details: 'payload' in (error as Record<string, unknown>) ? (error as { payload?: unknown }).payload : null };
  }
  if (error instanceof Error) {
    return { status: 500, message: error.message, details: null };
  }
  return { status: 500, message: 'A aparut o eroare neasteptata la publicarea in Storia.', details: null };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await publishPropertyToStoria({
      agencyId,
      propertyId,
      requestedByUid: uid,
    });
