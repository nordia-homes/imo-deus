...
export async function POST(
  ...
  try {
    const [{ campaignId }, { requireAgencyUserFromBearerToken }, { pauseMetaCampaignDraft }] = await Promise.all([
      context.params,
    ...
    const campaign = await pauseMetaCampaignDraft(agencyId, campaignId);
