...
const doc=await getDocument({data:new Uint8Array(fs.readFileSync('tmp/prezentare-proprietate-lux.pdf')),useSystemFonts:true}).promise;
const page=await doc.getPage(1);const viewport=page.getViewport({scale:2});
const canvas=createCanvas(Math.ceil(viewport.width),Math.ceil(viewport.height));
await page.render({canvasContext:canvas.getContext('2d'),viewport}).promise;
fs.writeFileSync('tmp/prezentare-proprietate-lux-final.png',canvas.toBuffer('image/png'));
