...
try {
  ...
  fs.writeFileSync('tmp/prezentare-proprietate-lux.html',html);
  await page.setContent(html,{waitUntil:'networkidle'});
  await page.pdf({path:'tmp/prezentare-proprietate-lux.pdf',format:'A4',printBackground:true,preferCSSPageSize:true,margin:{top:0,right:0,bottom:0,left:0}});
  await page.screenshot({path:'tmp/prezentare-proprietate-lux.png',fullPage:true});
  const doc=await PDFDocument.load(fs.readFileSync('tmp/prezentare-proprietate-lux.pdf'));
