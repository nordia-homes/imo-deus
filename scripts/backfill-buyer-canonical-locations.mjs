...
function buildCanonicalRef(row) {
  ...
  if (!county || !locality || !display) return null;
...
const catalogRefs = rawLocations.filter(isCatalogCandidate).map(buildCanonicalRef).filter(Boolean);

function resolveCanonicalLocation({ city, zone, label, location }) {
  ...
  for (const candidate of textCandidates) {
    ...
    const match = catalogRefs.find((entry) => normalize(entry.display).includes(candidateNorm) || normalize(entry.searchText).includes(candidateNorm));
    if (match) {
      return match;
    }
  }
...
function deriveCanonicalBuyerPreferences(contact) {
  ...
  function pushPreference(preference) {
    const key = [
      preference.preference,
      preference.scope,
      preference.location?.locationId || '',
      normalize(preference.locality || ''),
      normalize(preference.sourceText || ''),
    ].join('::');
  ...
  for (const zoneText of Array.isArray(contact.zones) ? contact.zones : []) {
...
async function main() {
  ...
  for (const agencyDoc of agenciesSnapshot.docs) {
    ...
    for (const contactDoc of contactsSnapshot.docs) {
      ...
      const nextPreferences = deriveCanonicalBuyerPreferences(contact);
      ...
      const previousSerialized = JSON.stringify(contact.locationPreferencesV2 || []);
      ...
      prepared += 1;
      console.log(`[${agencyDoc.id}] contact ${contactDoc.id}`, {
        name: contact?.name ?? null,
        previousCount: Array.isArray(contact.locationPreferencesV2) ? contact.locationPreferencesV2.length : 0,
        nextCount: nextPreferences.length,
        mode: shouldWrite ? 'write' : 'dry-run',
      });

      if (shouldWrite) {
        await contactDoc.ref.update({ locationPreferencesV2: nextPreferences });
        updated += 1;
      }
    }
  }
...
main().catch((error) => {
  console.error('Buyer canonical location backfill failed:', error);
  process.exit(1);
});
